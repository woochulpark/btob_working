				...
				if (topage >= 0 && topage < totalpages) {
					...
					if (initialized) {
						if (settings.onchange != null) {
							settings.onchange((currentPage + 1), recordStartIndex, recordsEndIndex, settings.params);
						}
					}
...
// 외부에서 호출
function pageing(elementId, total_record, page, params, callback){
	$('#'+elementId).smartpaginator({totalrecords: total_record, 
		recordsperpage: 10,
		theme: 'whitegray',
		controlsalways:false,
		display: 'single',
